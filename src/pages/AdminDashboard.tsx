...
const AdminDashboard = () => {
  ...
  const fetchAllMetrics = async () => {
    ...
    const [
      ...
      supabase.from("profiles_public" as any).select("id, first_name, last_name, role, created_at").order("created_at", { ascending: false }).limit(10),
      supabase.from("bookings").select("id, total_paid, platform_fee, status, created_at").order("created_at", { ascending: false }).limit(10),
      supabase.from("profiles_public" as any).select("created_at").gte("created_at", subDays(new Date(), 30).toISOString()).order("created_at", { ascending: true }),
      ...
      <header className="sticky top-0 z-50 border-b bg-card/80 backdrop-blur-lg">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
          <div className="flex items-center gap-3">
            <Link to="/" className="text-xl font-bold text-foreground">SubIn</Link>
            <Badge className="bg-destructive text-destructive-foreground text-[10px]">ADMIN</Badge>
          </div>
